...
describe("requirePermission", () => {
  ...
  it("grants a permission the role actually has (Admin -> agreement.finalize)", async () => {
    const admin = await prisma.user.findUnique({ where: { email: "admin@yahshua.test" } });
    const result = await requirePermission(fakeReq(tokenFor(admin)), "agreement.finalize");
  ...
  it("rejects with 403 when the role lacks the permission (Viewer -> agreement.finalize)", async () => {
    const viewer = await prisma.user.findUnique({ where: { email: "viewer@yahshua.test" } });
    const result = await requirePermission(fakeReq(tokenFor(viewer)), "agreement.finalize");
  ...
  it("rejects an unknown/unseeded permission key outright (typo-safe: no permission row means no access, never a silent pass)", async () => {
    const admin = await prisma.user.findUnique({ where: { email: "admin@yahshua.test" } });
    const result = await requirePermission(fakeReq(tokenFor(admin)), "agreement.this_key_does_not_exist");
    expect(result).toEqual({ ok: false, status: 403, error: "Forbidden" });
  });
});
